...
// Handling scroll event to close the liked menu
window.addEventListener('scroll', () => {
  closeLikedMenu();
});
// Handling click event outside the liked menu to close it
document.body.addEventListener('click', (event) => {
  const isClickedOutside = !likedMenu.contains(event.target) && event.target !== likeIcon;
  if (isClickedOutside) {
    closeLikedMenu();
  }
});
